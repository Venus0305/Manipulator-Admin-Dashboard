...
import type { ICustomerItem } from 'models/customer/types';
...
const CustomerList = () => {
  ...
  const { list: data, isFetching, pagination } = useList<ICustomerItem>(customerQuery.customerList);

  const customerList = data.map((item, index) => ({ ...item, key: `customer-item-${index}` }));

  const columns: ProColumns<ICustomerItem>[] = [
    {
      dataIndex: '_id',
      title: t('customer:id'),
    },
    ...
    },
    {
      ...
      valueEnum: {
        ...
        WITHDRAW: { text: 'Withdraw', status: 'Error' },
      },
    },
    {
      key: 'action',
      render: (_, data) => (
        <div className="flex gap-2">
          <Link to={`/customer/${data._id}/profile`}>
